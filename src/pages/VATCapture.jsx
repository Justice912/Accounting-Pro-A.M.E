...
import { Plus, Trash2, Receipt, Calculator, Download, RotateCcw } from 'lucide-react';
...
export default function VATCapture({ storageScope = 'default' }) {
  const storageKey = `ame-vat-capture-workbench:${storageScope}`;
  const [invoiceMeta, setInvoiceMeta] = useState({ ...EMPTY_META });
  ...
  useEffect(() => {
    try {
      ...
      if (parsed?.invoiceMeta) {
        setInvoiceMeta({ ...EMPTY_META, ...parsed.invoiceMeta });
      }
      if (Array.isArray(parsed?.items) && parsed.items.length > 0) {
        setItems(parsed.items.map((row) => ({ ...EMPTY_ITEM, ...row })));
      }
      if (parsed?.savedAt) setLastSavedAt(parsed.savedAt);
    } catch (err) {
      console.error('Failed to load VAT Capture draft:', err);
    }
  }, [storageKey]);
  ...
  useEffect(() => {
    try {
      ...
      console.error('Failed to save VAT Capture draft:', err);
    }
  }, [invoiceMeta, items, storageKey]);
  ...
  const handleReset = () => {
    setInvoiceMeta({ ...EMPTY_META });
    setItems([{ ...EMPTY_ITEM }]);
  };

  const handleExportCsv = () => {
    const rows = [
      ['Vendor Name', invoiceMeta.vendorName],
      ['Vendor VAT Number', invoiceMeta.vendorVatNumber],
      ['Invoice Number', invoiceMeta.invoiceNumber],
      ['Invoice Date', invoiceMeta.invoiceDate],
      ['Category', invoiceMeta.category],
      ['Notes', invoiceMeta.notes],
      [],
      ['Description', 'Amount Excl VAT', 'VAT Rate (%)', 'VAT Amount', 'Amount Incl VAT'],
      ...items.map((item) => {
        const base = parseAmount(item.amount);
        const rate = parseAmount(item.vatRate);
        const vat = base * (rate / 100);
        return [item.description, base.toFixed(2), rate.toFixed(2), vat.toFixed(2), (base + vat).toFixed(2)];
      }),
      [],
      ['Totals', totals.net.toFixed(2), '', totals.vat.toFixed(2), gross.toFixed(2)],
    ];
    ...
    const safeInvoiceNo = (invoiceMeta.invoiceNumber || 'vat-capture').replace(/[^\w-]+/g, '_');
    link.href = url;
    link.download = `${safeInvoiceNo}.csv`;
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white px-3 py-1.5 text-sm"
              >
          ...
          {lastSavedAt && (
            <p className="text-xs text-slate-400 mt-2">
              Auto-saved draft: {new Date(lastSavedAt).toLocaleString('en-ZA')}
            </p>
          )}
        </div>
