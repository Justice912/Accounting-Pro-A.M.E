...
export default function VATCapture() {
  ...
  const vat201Snapshot = useMemo(() => {
    return items.reduce((acc, item) => {
      ...
      if (rate === 15) {
        acc.field1StandardRated += base;
        acc.field1aOutputVat += vat;
      } else if (rate === 0 && invoiceMeta.category === 'zero-rated') {
        acc.field2ZeroRated += base;
      } else {
        acc.field3Exempt += base;
      }

      return acc;
    }, {
      field1StandardRated: 0,
      field1aOutputVat: 0,
      field2ZeroRated: 0,
      field3Exempt: 0,
    });
  }, [items, invoiceMeta.category]);
  ...
  return (
    <div className="h-full overflow-y-auto p-6 md:p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          <h2 className="text-sm font-semibold text-slate-700 uppercase tracking-wide mb-4">VAT201 Snapshot</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            <div className="rounded-lg border border-slate-200 p-3">
              <p className="text-slate-500 text-xs">Field 1: Standard rated supplies (excl VAT)</p>
              <p className="font-semibold text-slate-800">{toMoney(vat201Snapshot.field1StandardRated)}</p>
            </div>
            <div className="rounded-lg border border-slate-200 p-3">
              <p className="text-slate-500 text-xs">Field 1A: VAT on standard rated supplies</p>
              <p className="font-semibold text-slate-800">{toMoney(vat201Snapshot.field1aOutputVat)}</p>
            </div>
            <div className="rounded-lg border border-slate-200 p-3">
              <p className="text-slate-500 text-xs">Field 2: Zero-rated supplies</p>
              <p className="font-semibold text-slate-800">{toMoney(vat201Snapshot.field2ZeroRated)}</p>
            </div>
            <div className="rounded-lg border border-slate-200 p-3">
              <p className="text-slate-500 text-xs">Field 3: Exempt supplies</p>
              <p className="font-semibold text-slate-800">{toMoney(vat201Snapshot.field3Exempt)}</p>
            </div>
          </div>
        </section>
      </div>
