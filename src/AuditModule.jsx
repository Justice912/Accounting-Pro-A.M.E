...
function AuditReportsTab({ engagement, risks, findings }) {
  ...
  const reportLines = [
    ...
    "",
    "2. KEY OPEN FINDINGS",
    ...(openFindings.length ? openFindings.map((f, index) => `${index + 1}. [${f.ref}] ${f.title} (${FINDING_SEVERITY[f.severity]?.label || f.severity})`) : ["No open findings."]),
    "",
  ...
  const downloadPdfReport = () => {
    const pdfData = createSimplePdf(reportLines);
    downloadBlob(pdfData, "application/pdf", `audit-report-${(engagement.clientName || "client").replace(/\s+/g, "-").toLowerCase()}.pdf`);
  };
  ...
  const downloadLeadSchedule = (schedule, format) => {
    ...
    downloadBlob(pdfData, "application/pdf", `lead-schedule-${schedule.code.toLowerCase()}.pdf`);
  };
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: theme.textMuted, margin: "4px 0 0" }}>Professional audit reports and Draftworx-style lead schedules</p>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={downloadPdfReport} style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 14px", background: theme.primary, color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 600, fontFamily: "inherit" }}>
            <Download size={14} /> Download PDF
          </button>
          <button onClick={downloadWordReport} style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 14px", background: theme.accent, color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 600, fontFamily: "inherit" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <button onClick={() => setReportView("reports")} style={{ padding: "8px 14px", borderRadius: 8, border: `1px solid ${theme.border}`, background: reportView === "reports" ? theme.primaryGlow : theme.card, color: reportView === "reports" ? theme.primary : theme.textMuted, cursor: "pointer", fontFamily: "inherit", fontSize: 12 }}>Professional Reports</button>
        <button onClick={() => setReportView("leadSchedules")} style={{ padding: "8px 14px", borderRadius: 8, border: `1px solid ${theme.border}`, background: reportView === "leadSchedules" ? theme.primaryGlow : theme.card, color: reportView === "leadSchedules" ? theme.primary : theme.textMuted, cursor: "pointer", fontFamily: "inherit", fontSize: 12 }}>Lead Schedules</button>
      </div>
      ...
      {reportView === "reports" ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0,1fr))", gap: 12 }}>
          {LEAD_SCHEDULES.map((schedule) => (
            <div key={schedule.id} style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 12, padding: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: 8 }}>
                <div>
                  <div style={{ fontSize: 11, color: theme.primary, fontWeight: 700 }}>{schedule.code}</div>
                  <div style={{ fontSize: 14, fontWeight: 700 }}>{schedule.title}</div>
                  <div style={{ fontSize: 11, color: theme.textMuted, marginTop: 4 }}>{schedule.isa}</div>
                </div>
              </div>
              <p style={{ fontSize: 12, color: theme.textMuted, margin: "10px 0" }}>{schedule.objective}</p>
              <ul style={{ margin: "0 0 10px 18px", color: theme.textMuted, fontSize: 12 }}>
                {schedule.procedures.map((proc, idx) => <li key={idx}>{proc}</li>)}
              </ul>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => downloadLeadSchedule(schedule, "pdf")} style={{ flex: 1, padding: "8px 10px", borderRadius: 8, border: `1px solid ${theme.primary}`, background: "transparent", color: theme.primary, cursor: "pointer", fontFamily: "inherit", fontSize: 12 }}>PDF</button>
                <button onClick={() => downloadLeadSchedule(schedule, "word")} style={{ flex: 1, padding: "8px 10px", borderRadius: 8, border: `1px solid ${theme.accent}`, background: "transparent", color: theme.accent, cursor: "pointer", fontFamily: "inherit", fontSize: 12 }}>Word</button>
              </div>
            </div>
          ))}
        </div>
