...
## 8) One-command merge + deploy helper
This repo now includes:
```bash
npm run release:merge-deploy
```
- Default mode is dry-run (prints commands only).
- To execute for real:
```bash
node scripts/merge-prs-and-deploy.mjs --execute
```
This workflow merges PR #49 then #52, updates `main`, and runs Vercel production deploy.
